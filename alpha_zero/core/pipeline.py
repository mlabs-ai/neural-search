...
def run_selfplay_actor_loop(
    ...
    log_level: str,
    var_ckpt: mp.Value,
    var_resign_threshold: mp.Value, # type: ignore
    ckpt_event: mp.Event,
    stop_event: mp.Event,
) -> None:
...
def compute_supervised_losses(network, device, transitions, argument_data):
    state, target_pi, target_v = transitions
    state = state.to(device=device, dtype=torch.float32, non_blocking=True)
    # [B, num_actions]
    target_pi = target_pi.to(device=device, dtype=torch.float32, non_blocking=True)
    # [B, ]
    target_v = target_v.to(device=device, dtype=torch.float32, non_blocking=True)
